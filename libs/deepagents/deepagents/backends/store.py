...
class StoreBackend(BackendProtocol):
    ...
    def ls(self, path: str) -> LsResult:
        ...
        # Use database-side filtering for efficiency
        items = self._search_store_paginated(
            store, 
            namespace, 
            filter={"key": {"$gte": search_prefix, "$lt": search_prefix + "\uffff"}}
        )
        ...
        for item in items:
            ...
            if "/" in relative:
                ...
                subdirs.add(f"{search_prefix}{subdir_name}/")
                continue
